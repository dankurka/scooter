//////////////////////////////////////////////////////////////////
//
//   $Header: /cvs/Controller\040Board/AVR-Code/Testsuite/main.c,v 1.26 2007/09/23 22:56:59 rainer Exp $
...
void print_status()
{
	...
	uart_puts_P("-- Controllerboard $Revision: 1.26 $ online --\n\r");
	uart_puts_P("Initialized UART-Interface: COM 1\n\r");
	uart1_puts_P("<S 91 $Revision: 1.26 $]\n\r");
}  
int main( void )
{ 
	...
	init_beeper();
	uart_puts_P("Initialized beeper\n\r");
	...
	while(1)
	{
		...
		{
			handle_display_com(); /*nop*/
			/* read battery volatage from time to time */
			if( systime_millisec == 0 ) /* if this is missed most of the time, its ok */
			{				
				adc_sel_channel(2);
			 	battery_samples[battery_idx++] = read_adc();
			 	battery_idx &= (BAT_SAMPLES_BUFSIZE-1);
			}
